...
func (db *DB) Get(opts *ReadOptions, key []byte) ([]byte, error) {
	...
	return charToByte(cValue, cValLen), nil
}
...
// NewIteratorsCF returns Iterators over the the database and column families
// that uses the ReadOptions given.
// len of the returned iterators is the len of cfs.
func (db *DB) NewIteratorsCF(opts *ReadOptions, cfs []*ColumnFamilyHandle) ([]*Iterator, error) {
	...
	citrs := make([]*C.rocksdb_iterator_t, len(cfs))
	for i, cf := range cfs {
		ccfs[i] = cf.c
	}
	...
	itrs := make([]*Iterator, len(cfs))
	for i, citr := range citrs {
		itrs[i] = NewNativeIterator(unsafe.Pointer(citr))
	}
